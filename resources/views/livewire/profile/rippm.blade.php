                    ...
                    <div class="card mb-4">
                        <div class="card-header justify-content-between align-items-center d-flex">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <a href="{{ route('rippm.add') }}" wire:navigate class="btn btn-primary btn-sm"><i class="ri-add-line"></i> Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
                            @foreach ($rippm as $index => $item)
                                <form wire:submit.prevent="update({{ $item['id'] }})" x-data="{ editing: false, confirmDelete: false }" wire:key="rippm-row-{{ $item['id'] }}">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <input type="text" class="form-control @error('rippm.' . $index . '.rippm_no_persetujuan') is-invalid @enderror"
                                                wire:model="rippm.{{ $index }}.rippm_no_persetujuan" :disabled="!editing">
                                            @error('rippm.' . $index . '.rippm_no_persetujuan')
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <input type="date" class="form-control @error('rippm.' . $index . '.rippm_tgl_persetujuan') is-invalid @enderror"
                                                wire:model="rippm.{{ $index }}.rippm_tgl_persetujuan" :disabled="!editing">
                                            @error('rippm.' . $index . '.rippm_tgl_persetujuan')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-md-5 mb-3">
                                            ...
                                            <input type="number" class="form-control @error('rippm.' . $index . '.rippm_keterangan') is-invalid @enderror"
                                                wire:model="rippm.{{ $index }}.rippm_keterangan" :disabled="!editing">
                                            @error('rippm.' . $index . '.rippm_keterangan')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
